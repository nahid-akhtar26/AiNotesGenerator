...
const History = () => {
  ...
  const openNotes=async(notesId)=>{
    setLoading(true)
    setActiveNoteId(notesId)
    try {
      const res=await axios.get(serverUrl+`/api/notes/${notesId}`,{withCredentials:true})
  ...
  return (
    <div className="min-h-screen bg-linear-to-bt from-gray-100 to-gray-200 px-6 py-8">
      <motion.header
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          >
            ...
            <motion.span
              ...
              whileTap={{ scale: 0.97 }}
              className="ml-2 h-5 w-5 flex items-center justify-center rounded-full bg-white  text-sx font-bold"
            >
              ➕
            </motion.span>
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <ul className="space-y-2">
                  {
                    topics.map((t,i)=>(
                      ...
                      `}>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2 text-xm">

                          {
                            t.classLevel && 
                            <span className="px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300">ClassLevel: {t.classLevel}</span>
                          }

                           {
                            t.examType && 
                            <span className="px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300">Exam Type: {t.examType}</span>
                          }
        ...
        className="lg:col-span-3 rounded-2xl bg-white shadow-[0_15px_40px_rgba(0,0,0,0.15)] p-6 min-h-[75vh]"
          >
            ...
            {
              !loading && selectedNotes && <FinalResult result={selectedNotes}/>
            }
